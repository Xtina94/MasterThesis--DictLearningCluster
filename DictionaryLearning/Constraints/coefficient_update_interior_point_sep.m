function [alpha, cpuTime, param] = coefficient_update_interior_point_sep(Data,CoefMatrix,param,sdpsolver,g_ker,p)
...
eval(['Lambda = param.lambda_power_matrix',num2str(p),';']);
thresh = param.thresh;
...
% % %         + (BB*alpha >= ((c-0.1*epsilon)*ones(lb,1)));
    ...
    %otherwise we're having a low frequency kernel
    ...
    for i = 2:param.S
        ...
        if i > 1 %it means that we're facing a high frequency kernel and we do not put constraints on it
            BA = kron(eye(S-1),Lambda);
            BB = kron(ones(1,S-1),Lambda);
            la = length(BA*alpha(22:end));
            lb = length(BB*alpha(22:end));
    ...
    F = (B1*alpha <= c*ones(l1,1))...
        ...
        + (B3*alpha <= 0.1011)...
        + (B3*alpha >= 0.0754)...
        + (BA*alpha(22:end) <= c*ones(la,1))...
        + (-BA*alpha(22:end) <= -0.001*epsilon*ones(la,1))...
        + (BB*alpha(22:end) <= (c+0.1*epsilon)*ones(lb,1))...
        + (-BB*alpha(22:end) <= ((c-0.1*epsilon)*ones(lb,1)));
end
