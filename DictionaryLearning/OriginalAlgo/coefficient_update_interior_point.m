function [alpha, cpuTime] = coefficient_update_interior_point(Data,CoefMatrix,param,sdpsolver,p)
...
eval(['high_freq_thr = ceil((length(param.lambda_sym',num2str(p),'))/2);']); 
...
if param.big_epoch < 51 % (In this way I'm keeping the algo as the original one, cause param.numIterations is always < that 52 (20 now))
    F = (BA*alpha <= c*ones(la,1))...
        + (-BA*alpha <= -0.001*epsilon*ones(la,1))...
        + (BB*alpha <= (c+0.1*epsilon)*ones(lb,1))...
        + (-BB*alpha <= ((c-0.1*epsilon)*ones(lb,1)));
